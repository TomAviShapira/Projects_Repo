...
def upBlock4(in_planes, out_planes):
    ...
    block = nn.Sequential(
        ...
        nn.ConvTranspose1d(in_planes, out_planes, kernel_size=kernel_length, stride=stride, padding=19,
                           output_padding=1),
...
def upBlock2(in_planes, out_planes):
    ...
    block = nn.Sequential(
        # nn.Upsample(scale_factor=4, mode='nearest'),
        # conv3x1(in_planes, out_planes),
        nn.ConvTranspose1d(in_planes, out_planes, kernel_size=kernel_length, stride=stride, padding=20,
                           output_padding=1),
        nn.BatchNorm1d(out_planes),
        # nn.ReLU(True)
        nn.PReLU())
...
class STAGE1_G(nn.Module):
    ...
    def forward(self, inputWAV):
        # c_code, mu, logvar = self.ca_net(text_embedding)

        c_code = self.encoder_model(inputWAV)
        c_code_ = c_code

        c_code = self.cond_net(c_code)  # was text_embedding instead of c_code
        ...
        return None, fake_RIR, c_code_  # c_code_
